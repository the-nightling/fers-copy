  ...
  rsFloat QuantizeWindow(rs::rsComplex *data, unsigned int size)
  {
    ...
    for (unsigned int i = 0; i < size; i++) {
      ...
      if (std::fabs(data[i].imag()) > max)
        ...
        if (boost::math::isnan(data[i].real()) || (boost::math::isnan(data[i].imag())))
